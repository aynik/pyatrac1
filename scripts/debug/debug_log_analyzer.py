...
class LogParser:
    ...
    def __init__(self):
        # Regex pattern to parse the new detailed log entries
        # Modified to handle negative frame numbers like FR-01, FR-02
        self.log_pattern = re.compile(
            r'\[([^\]]+)\]\[([^\]]+)\]\[([^\]]+)\]\[([^\]]+)\]\[CH(\d+)\]\[FR(-?\d+)\](?:\[BAND_([^\]]+)\])?\s*([^:]+):\s*([^=]+)=([^\|]+)\s*\|META:\s*([^\|]+)\s*\|SRC:\s*(.+)'
        )
        
        # Alternative pattern for simpler new format logs  
        self.simple_pattern = re.compile(
            r'\[([^\]]+)\]\[([^\]]+)\]\[([^\]]+)\]\[([^\]]+)\]\[CH(\d+)\]\[FR(-?\d+)\](?:\[BAND_([^\]]+)\])?\s*([^:]+):\s*([^=]+)=([^\s]+)'
        )
    ...
    def parse_entry(self, line: str) -> Optional[LogEntry]:
        ...
        # Try the full detailed pattern first
        ...
        # Try the simpler pattern for newer logs without full metadata
        match = self.simple_pattern.match(line)
        if match:
            timestamp, impl, file_line, function, channel, frame, band, stage, data_type, values = match.groups()
